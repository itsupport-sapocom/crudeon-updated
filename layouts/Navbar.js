...
function Navbar() {
  ...
  return (
    <header className='w-full px-4 py-2 h-[65px] sticky top-0 left-0 z-[90] shadow-md bg-white'>
      ...
      {/* PC & Laptop Screen */}
      <div className={`max-w-200 mx-auto flex justify-between gap-4 items-center h-full max-md:fixed max-md:right-0 max-md:top-0 max-md:bg-white max-md:h-screen max-md:w-[250px] max-md:flex-col max-md:p-4 max-md:items-start z-[92] max-md:shadow-nav max-md:transition-all ${!showNav && 'max-md:right-[-300px]'}`}>
        {/* Logo */}
        <Link href='/' passHref className='max-md:hidden'>
          <Image src={logo} width={380} className='h-auto w-auto' alt='header_logo' />
        </Link>
